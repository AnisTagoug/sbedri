...
$query = "SELECT * FROM products ORDER BY idP DESC";
...
if($statement->execute())
{
 ...
 foreach($result as $row)
 {
  ...
  <div class="col-md-3" style="margin-top:12px;margin-bottom:12px;">  
            <div style="border:1px solid #ccc; border-radius:5px; padding:16px; height:80%;" align="center" id="product_'.$row["idP"].'">
             <img src="../img/'.$row["imgP"].'" style="width:80%;height:80%;" class="img-responsive" /><br />
             <h4 class="text-info">
                        <div class="checkbox">
                              <label><input type="checkbox" class="select_product" data-product_id="'.$row["idP"].'" data-product_name="'.$row["nameP"].'" data-product_price="'.$row["priceP"] .'" value="">'.$row["nameP"].'</label>
                        </div>
                  </h4>
             <h4 class="text-danger">'.$row["priceP"] .' TND</h4>
            </div>
        </div>
  ';
 }
